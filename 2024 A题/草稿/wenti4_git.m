...
theta_t = 3*2*pi * ones(1,step + 1); % 0-300s所有的theta取值都在这里了，我们再用坐标变换再求其余坐标即可
...
for tt = 1:step
    ...
    % === 段1：阿基米德盘入 ===
    if isnan(time1)
        theta_t(tt+1) = theta_t(tt) - (1/sqrt(b^2 + (a + b * theta_t(tt))^2)) * delta_t;
    ...
    elseif isnan(time2)
        ...
        for ii = 2:head_num + 1
            ...
            if flag_AC(ii) == 1
                % 如果第ii个把手也在AC上
                theta_O1_tt(ii) = theta_calculate4(xx_tt(ii - 1), yy_tt(ii - 1), O, O1, R_large, l, theta_O1_tt(ii - 1) , gamma1);
            ...
            if flag_in(ii) == 1
                ...
                if flag_AC(ii - 1) == 1
                    % 前一个把手在AC上
                    theta1_handle_tt(ii) = theta_calculate1(xx_tt(ii - 1), yy_tt(ii - 1), l, a, b, theta_i);
                    xx_tt(ii) = (a + b * theta1_handle_tt(ii)) * cos(theta1_handle_tt(ii));
                    yy_tt(ii) = (a + b * theta1_handle_tt(ii)) * sin(theta1_handle_tt(ii));
                ...
                if flag_in(ii - 1) == 1
                    % 前一个把手也在盘入曲线
                    [xx_tt(ii), yy_tt(ii)] = theta_calculate5(xx_tt(ii - 1), yy_tt(ii - 1), a, b, l);
                    continue;
                end
    ...
    elseif isnan(time3)
        ...
        if theta_O2_tt(1) >= gamma2 + theta2
            time3 = time;
            theta_out_tt(1) = theta_i;  % 初始化中心对称段起始角度
        end
        ...
        for ii = 2:head_num + 1
            ...
            if flag_BC(ii) == 1
                ...
                theta_O2_tt(ii) = theta_calculate4(xx_tt(ii - 1), yy_tt(ii - 1), O, O2, R, l, gamma2 , theta_O2_tt(ii - 1));
            ...
            if flag_AC(ii) == 1
                ...
                if flag_BC(ii - 1) == 1
                    % 如果前一个把手在BC上
                    theta_O1_tt(ii) = theta_calculate4(xx_tt(ii - 1), yy_tt(ii - 1), O, O1, R_large, l, gamma1 - theta1 , gamma1);
                end
        ...
        for ii = 2:head_num + 1
            ...
            if flag_out(ii) == 1
                % 第ii把手在盘出曲线上
                theta_out_tt(ii) = theta_calculate3(theta_out_tt(ii - 1), l, a, b);
                xx_tt(ii) = (a + b * theta_out_tt(ii)) * cos(theta_out_tt(ii) + pi);
                yy_tt(ii) = (a + b * theta_out_tt(ii)) * sin(theta_out_tt(ii) + pi);
            end
            ...
            if flag_BC(ii) == 1
                ...
                if flag_out(ii - 1) == 1
                    % 前把手在盘出曲线上
                    theta_O2_tt(ii) = theta_calculate4(xx_tt(ii - 1), yy_tt(ii - 1), O, O2, R, l, gamma2 , gamma2 + theta2);
                end
